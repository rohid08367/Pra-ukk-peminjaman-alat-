...
if (isset($_POST['tambah'])) {
    ...
    if($_FILES['gambar']['name'] != ''){
        ...
        move_uploaded_file($tmp,"../assets/img_alat/".$nama_file);

        $gambar = $nama_file;
    }

    mysqli_query($conn,"
        INSERT INTO alat (nama_alat,gambar,kategori_id,stok,status,denda_per_hari)
        VALUES ('$nama','$gambar','$kategori','$stok','$status','$denda')
    ");
...
if (isset($_POST['update'])) {
    ...
    if($_FILES['gambar']['name'] != ''){
        ...
        $gambar_update = ", gambar='$nama_file'";
    }
...
if (isset($_GET['hapus'])) {
    ...
    $ambil = mysqli_fetch_assoc(mysqli_query($conn,"
        SELECT gambar FROM alat WHERE id='$id'
    "));

    if($ambil['gambar']!=''){
        unlink("../assets/img_alat/".$ambil['gambar']);
    }
...
<?= ($edit && $edit['status']=='tersedia')?'selected':'' ?>>
Tersedia
</option>

<option value="rusak"
<?= ($edit && $edit['status']=='rusak')?'selected':'' ?>>
Rusak
</option>
...
<img src="../assets/img_alat/<?= $edit['gambar'] ?>"
...
<img src="../assets/img_alat/<?= $row['gambar'] ?>"
